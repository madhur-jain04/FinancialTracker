...
// const API_BASE_URL = '/api'; // Placeholder, actual calls are mocked.
...
function App() {
  ...
  // Initial Load Effect and Data Setup
  // useEffect(() => {
  ...
  //     setView('auth');
  //   }
  // }, [user?.token]); // Rerun if user state changes
  ...
  // if (!user) {
  //   // --- Auth View (Unchanged, remains appealing) ---
  //   return (
  //     <div className="min-h-screen font-sans bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center p-4">
  //       <div className="flex flex-col lg:flex-row items-center justify-between w-full max-w-4xl bg-white rounded-3xl shadow-3xl overflow-hidden">
  //         <div className="w-full lg:w-1/2 p-8 hidden lg:block">
  //           <img
  //             src="https://placehold.co/600x400/818cf8/ffffff?text=Financial+Planning"
  ...
  //             </div>
  //             <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Finance Assistant</h1>
  //           </div>
  //           <p className="text-gray-500 mb-8">Track, manage, and understand your finances</p>
  //           <div className="bg-white p-6 rounded-2xl border border-gray-100">
  //             <h2 className="text-xl font-semibold text-gray-700 mb-4 text-center">Welcome</h2>
  //             <AuthForm onLogin={handleLogin} />
  //             <p className="text-center text-xs text-gray-400 mt-6">Demo app for prototyping purposes only.</p>
  //           </div>
  //         </div>
  //       </div>
  //     </div>
...
// import { LogOut, TrendingUp, Wallet, User, PlusCircle, Receipt, FileText, LayoutDashboard, Search, Filter } from 'lucide-react';

// // Import all required components from their new files
// import AuthForm from '../components/AuthForm.jsx';
// import TransactionForm from '../components/TransactionForm.jsx';
// import TransactionList from '../components/TransactionList.jsx';
// import ChartContainer from '../components/ChartContainer.jsx';
// import ReceiptUploader from '../components/ReceiptUploader.jsx';
// import PdfUploader from '../components/PdfUploader.jsx'; // Assuming you create this one
// import SummaryCards from '../components/SummaryCards.jsx';

// // --- Global Setup ---
// const API_BASE_URL = 'http://localhost:5000/api';

// // Create an instance of Axios for setting a global base URL and interceptors
// const api = axios.create({
//   baseURL: API_BASE_URL,
// });

// // Axios Request Interceptor: Attach the token to every request
...
// // Helper for conditional amount coloring (can remain here or be moved to utils)
// const getAmountColor = (type) => (type === 'Income' ? 'text-green-600' : 'text-red-600');
...
//   const handleLoginSuccess = useCallback((userData, jwtToken) => {
//     // Save token and user data on successful login/register
...
//   const verifyAuth = useCallback(async (current_token) => {
//     if (!current_token) return handleLogout();
    
//     try {
//       // Use a protected route (like getting user details or status) to verify the token
...
//       localStorage.setItem('userInfo', JSON.stringify(verifiedUser)); // Update user info
//       setView('dashboard');
//     } catch (error) {
...
//   useEffect(() => {
//     // Attempt to verify token on component mount
//     verifyAuth(token);
    
//     // Set view to 'auth' if no token exists after initial checks
...
//     if (!token || !user) return; // Prevent fetch if not authenticated
...
//     } catch (error) {
//       console.error("Error fetching transactions:", error);
//       alert("Failed to load transactions. Please check your backend.");
//     } finally {
//       setLoading(false);
//     }
//   }, [token, user]);

//   // Initial load of transactions upon successful authentication
//   useEffect(() => {
//     if (user && view === 'dashboard') {
//       fetchTransactions();
//     }
//   }, [user, fetchTransactions, view]);

//   const handleNewTransaction = useCallback((newTransaction) => {
//     // Optimistic UI update, then trigger a full re-fetch for accuracy
//     setTransactions(prev => [newTransaction, ...prev]);
//     // A slight delay ensures the backend has time to process the new transaction
...
//             <h1 className="text-center text-4xl font-extrabold mb-8 bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
//                Finance Assistant
//             </h1>
//           <AuthForm 
...
//     { id: 'transactions', label: 'History', icon: Wallet },
//     { id: 'analytics', label: 'Analytics', icon: TrendingUp },
//     { id: 'receipt', label: 'Scan Receipt', icon: Receipt },
...
//             <div className="flex items-center space-x-3">
//               <User className="w-5 h-5 text-gray-500 hidden sm:inline" />
//               <span className="text-sm font-medium text-gray-700 hidden sm:inline">{user?.email || 'User'}</span>
//               <button
//                 onClick={handleLogout}
//                 className="flex items-center space-x-1 px-3 py-1.5 text-xs rounded-lg border border-gray-300 text-gray-600 hover:bg-red-50 hover:text-red-600 transition-colors"
//               >
...
//             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
//               <SummaryCards summary={summary} transactions={transactions} cardClass={cardClass} />
//             </div>
...
//               </h2>
//               <TransactionList
//                 transactions={transactions.slice(0, 5)} // Only show recent 5 on dashboard
//                 loading={loading}
//                 getAmountColor={getAmountColor}
//                 // No filters needed for the dashboard preview
//               />
//             </div>
//           </div>
//         )}
...
//           <div className="max-w-2xl mx-auto">
//             <div className={cardClass}>
//               <h2 className="flex items-center space-x-3 text-2xl font-bold text-gray-800 border-b border-gray-100 pb-4 mb-6">
//                 <PlusCircle className="w-8 h-8 text-blue-600" />
//                 <span>Add Manual Transaction</span>
//               </h2>
//               <TransactionForm 
//                 api={api} // Pass the configured axios instance
...
//               loading={loading}
//               getAmountColor={getAmountColor}
//               onFilterChange={fetchTransactions} // Pass the fetch function to reload with new filters
//             />
//           </div>
//         )}
